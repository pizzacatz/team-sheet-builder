...
- After a blocked attempt, an `Ignore all errors` escape hatch appears below the action row. It downloads the combined PDF as-is, validation errors and all — for players who need the sheet anyway (its tooltip warns the sheet may be rejected at check-in).
...
Actions reveal the outstanding errors and jump to the first one instead of proceeding while validation contains errors. After that first blocked attempt, an `Ignore all errors` button appears and downloads the combined PDF without fixing anything.
