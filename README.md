...
- The viewer answers `/api/*` requests from any origin, so no CORS allowlist is needed on either side.
...
- `Email to TO`: opens a `mailto:` draft with no recipient (player info + team link in the body).
