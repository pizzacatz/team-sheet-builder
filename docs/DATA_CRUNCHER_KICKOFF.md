...
Paste the block below to start the new session. It assumes you can point that
session at this repo (or that you have copied the referenced files across).
...
Authoritative contract: read `docs/DATA_CRUNCHER_SPEC.md` from the Team Sheet
Builder repo FIRST — it defines exactly what is embedded in the PDFs and what to
build. Also copy these reference artifacts from that repo:
  - docs/DATA_CRUNCHER_SPEC.md   (the full spec — start here)
  - docs/CODE_INDEX.md           (QR number registry contract, if you do QR)
  - scripts/decode_team_data.mjs (working Node decoder for both carriers — port it)
  - src/pdf/teamDataCode.ts       (exact wire format: sentinels, field order, widths)
  - src/data/regulation-mb/*.json (species/moves/abilities/items/stat-alignments,
